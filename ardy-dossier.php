...
function ardy_genera_dossier(PDO $db, string $sessionId, bool $perCliente = false, bool $senzaChat = false): ?string {
    ...
    $righe = [
        ...
        'Budget'       => $c['budget']    ?? '',
        'Prossimo appuntamento' => ardy_dossier_data($c['sopralluogo_at'] ?? ''),
    ...
    // Consulenza Interior Design: il riepilogo scritto da Sole nella webchat dedicata
    // (o a mano in dashboard). Client-safe: racconta i gusti del cliente stesso.
    if (!empty($c['interior_design_attivo'])) {
        $md .= "\n## Consulenza Interior Design\n";
        if (trim((string) ($c['interior_design_note'] ?? '')) !== '') {
            $md .= ardy_dossier_tronca($c['interior_design_note'], 1200) . "\n";
        } else {
            $md .= "_Sezione attiva, riepilogo non ancora compilato._\n";
        }
        // Campi storici: fino a lug 2026 stile/colori/luce/budget erano voci separate,
        // poi sostituite dal riepilogo unico. Le schede vecchie possono averli ancora
        // valorizzati: li mostriamo per non far sparire dati già raccolti.
        $storici = [
